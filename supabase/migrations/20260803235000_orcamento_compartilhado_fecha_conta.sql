-- =========================================================================
-- A carta compartilhada tem que FECHAR a conta
--
-- Na primeira versão o documento mostrava "Custo de produção R$ 40.600",
-- depois "Taxa da produtora (30%)" e "Imposto (12%)" sem número nenhum, e
-- terminava em "Total R$ 58.650". Quem recebe vê R$ 18.050 aparecendo do
-- nada — e a pessoa a quem se manda isso é justamente a que vai conferir a
-- conta. Um orçamento que não fecha é pior que nenhum.
--
-- Passa a devolver os VALORES de margem, comissão e imposto, calculados
-- aqui com a mesma regra do editor:
--
--   base da margem = só as linhas que NÃO estão "fora da taxa"
--   sub-total 2    = custo de produção + margem
--   comissão       = sobre o sub-total 1 ou 2, conforme o orçamento
--   imposto        = sobre (sub-total 2 + comissão)
--
-- Cada valor continua obedecendo a flag do link: sem "impostos", o número do
-- imposto não sai do banco — o que sai é o total, que o cliente veria de
-- qualquer jeito.
--
-- Partiu da definição vigente do arquivo 20260803230000 (CREATE OR REPLACE
-- aceita qualquer corpo caladinho: reescrever de memória apaga o que veio
-- depois, e isso já custou caro aqui).
-- =========================================================================
