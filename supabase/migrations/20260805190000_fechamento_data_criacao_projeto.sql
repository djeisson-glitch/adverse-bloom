-- =========================================================================
-- "Esse projeto é de junho, por que está no fechamento de julho?"
--
-- Estava certo, e a pergunta é boa: o fechamento por HORAS cobra o trabalho
-- feito NO MÊS, não os projetos nascidos no mês. Um projeto de 16/06 que
-- consumiu 1h40 no dia 29/07 é cobrado em julho — e se consumir mais em
-- agosto, é cobrado de novo em agosto. Mudar a data de criação não move (nem
-- deve mover) a hora de mês: ela iria parar num fechamento já faturado.
--
-- O que faltava era DIZER isso na tela. A lista "horas por projeto" mostrava
-- só o nome, e o nome do projeto carrega a data no prefixo (#20261206 = 12
-- de junho) — então batia o olho e parecia erro de mês, todo fechamento.
--
-- Agora cada linha leva a data de criação do projeto junto, e a tela marca as
-- que vêm de mês anterior. Nenhum número muda; muda o que dá pra conferir.
--
-- Partiu da definição vigente do 20260804041000 — copiada inteira, alterado
-- só o bloco `_por_projeto`.
-- =========================================================================
  ...
  LOOP
    ...
    FROM (
      SELECT jsonb_build_object(
               ...
               'criacao', COALESCE(p.criado_em, p.created_at),
