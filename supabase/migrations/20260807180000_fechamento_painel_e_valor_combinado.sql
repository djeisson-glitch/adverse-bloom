-- =========================================================================
-- O fechamento aprende o valor combinado, e ganha um painel
--
-- Duas coisas na mesma função, porque as duas mexem em `por_projeto`:
--
-- 1. VALOR COMBINADO POR PROJETO (`projects.valor_fechamento`). Quando um job
--    tem valor acordado — o do orçamento, tipicamente —, ele entra no
--    fechamento por esse valor e as horas dele saem da conta por hora. Vale
--    nos três baldes: o acordo é o acordo, esteja o job onde estiver.
--
--    modelo horas    (horas do balde − horas dos jobs combinados) × valor-hora
--                    + soma dos valores combinados
--    modelo tabela   as peças do job combinado não viram linha de preço; o
--                    job soma por fora. Somar as duas coisas cobraria duas
--                    vezes o mesmo trabalho.
--
--    GREATEST(0, …) na subtração: o único jeito de dar negativo é um bug de
--    recorte, e subtotal negativo vira desconto na nota do cliente antes de
--    alguém perceber.
--
-- 2. `por_projeto` VIRA O PAINEL. Antes só listava o balde 'mensal' — então
--    marcar um projeto como nota separada o fazia sumir da lista, junto com
--    o botão que o traria de volta. Agora traz todos os baldes, com o valor,
--    o orçamento vinculado, o override e AS PEÇAS, cada uma com o seu balde
--    efetivo.
--
--    Esse último campo é o que faltava hoje: os projetos 0020 e 0203 do Sul
--    Minas estavam como nota separada no projeto e com 'mensal' gravado na
--    PEÇA — que vence o projeto, pela regra — e a nota saiu R$ 0,00 sem
--    nenhuma tela dizer por quê.
--
-- Partiu da definição vigente (20260807110000).
-- =========================================================================
...
BEGIN
  ...
  LOOP
    ...
    SELECT
      COALESCE(SUM(te.duration_min) FILTER (WHERE x.balde = 'mensal'), 0) / 60.0,
      COALESCE(SUM(te.duration_min) FILTER (WHERE x.balde = 'mensal_separado'), 0) / 60.0
    ...
    FROM (
      SELECT jsonb_build_object(
               ...
               'orcamento_valor', fin.sold_value,
    ...
    IF c.modelo = 'horas' THEN
      ...
      _subtotal     := ROUND(GREATEST(0, _h_tot - _h_ovr_mensal) * c.valor_hora, 2) + _ovr_mensal;
      _sep_subtotal := ROUND(GREATEST(0, _h_tot_sep - _h_ovr_sep) * c.valor_hora, 2) + _ovr_sep;
    ...
    ELSIF c.modelo = 'tabela' THEN
      _valor_hora := 0;
      _sep_regra := 'preço de tabela das peças';
    ...
    _sep_total    := _sep_subtotal + _sep_margem + _sep_imposto;
...
DO $medicao$
DECLARE
  r record; mes date := date '2026-07-01'; fim date := date '2026-08-01';
  a numeric; b numeric; d numeric; total numeric;
  n_paineis int := 0; sem_peca int := 0;
BEGIN
  -- 1. A partição de horas em três baldes continua exata.
  FOR r IN SELECT DISTINCT cf.client_id, cl.name
             FROM public.client_faturamento cf JOIN public.clients cl ON cl.id = cf.client_id
            WHERE cf.modelo <> 'nenhum'
  LOOP
    SELECT
      COALESCE(SUM(te.duration_min) FILTER (WHERE x.balde = 'mensal'), 0),
      COALESCE(SUM(te.duration_min) FILTER (WHERE x.balde = 'mensal_separado'), 0),
      COALESCE(SUM(te.duration_min) FILTER (WHERE x.balde = 'avulso'), 0),
      COALESCE(SUM(te.duration_min), 0)
      INTO a, b, d, total
    FROM public.time_entries te
    JOIN public.projects p ON p.id = te.project_id
    LEFT JOIN public.deliverables_criacao dc     ON dc.id = te.deliverable_id
    LEFT JOIN public.deliverables_faturamento df ON df.id = te.deliverable_id
    CROSS JOIN LATERAL (SELECT COALESCE(df.faturamento_efetivo, p.faturamento, 'mensal') AS balde) x
    WHERE p.client_id = r.client_id AND te.billable
      AND COALESCE(dc.criacao_efetiva, p.criado_em, p.created_at) >= mes
      AND COALESCE(dc.criacao_efetiva, p.criado_em, p.created_at) <  fim;
    IF a + b + d <> total THEN
      RAISE EXCEPTION 'particao quebrada em %: %+%+% <> %', r.name, a, b, d, total;
    END IF;
  END LOOP;

  -- 2. O painel tem que enxergar os projetos que estão FORA do mensal — era
  --    justamente esses que sumiam da lista antiga, levando o botão junto.
  PERFORM public.gerar_faturamento_mensal(mes);
  SELECT count(*) INTO n_paineis
    FROM public.faturamento_mensal fm, LATERAL jsonb_array_elements(fm.detalhe->'por_projeto') x
   WHERE fm.ref_mes = mes AND x->>'balde' <> 'mensal';
  IF n_paineis = 0 THEN
    RAISE EXCEPTION 'o painel nao listou nenhum projeto fora do mensal — a lista voltou a filtrar';
  END IF;

  -- 3. E tem que trazer as peças com o balde de cada uma, que é o dado que
  --    explica uma nota separada em zero.
  SELECT count(*) INTO sem_peca
    FROM public.faturamento_mensal fm, LATERAL jsonb_array_elements(fm.detalhe->'por_projeto') x
   WHERE fm.ref_mes = mes AND NOT (x ? 'pecas');
  IF sem_peca > 0 THEN RAISE EXCEPTION '% linha(s) do painel sem a lista de peças', sem_peca; END IF;

  RAISE NOTICE 'particao ok · painel com % projeto(s) fora do mensal visiveis', n_paineis;
END $medicao$;
