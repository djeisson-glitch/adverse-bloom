-- =========================================================================
-- Duas telas, duas datas de criação pro MESMO projeto
--
-- Ontem o fechamento passou a mostrar a data de criação. Só que ele calculava
-- `coalesce(p.criado_em, p.created_at)` — só o projeto — enquanto a tela de
-- Entregas do mês corta o mês pela view `deliverables_criacao`, que usa
-- `coalesce(p.criado_em, d.criado_em, p.created_at, d.created_at)`.
--
-- Resultado medido, cliente Sicredi Sul Minas:
--
--   #20263006_DIA_INTERNACIONAL   fechamento: "criado 03 de jul."
--                                 entregas:    30/06  → nem aparece em julho
--
-- A peça tinha `criado_em` ajustado à mão pra 30/06 (bate com o #20263006 do
-- nome); o `created_at` do projeto, 03/07, é quando ele entrou no sistema. A
-- view estava certa — quem estava fora da regra era o fechamento, e recém.
--
-- Aqui o fechamento passa a LER a mesma view. Uma regra só: se as duas telas
-- discordam da data, uma delas está mentindo, e não dá pra saber qual.
--
-- Partiu da definição vigente do 20260805190000 — alterado só o bloco
-- `_por_projeto`. Nenhum valor muda: `criacao` não entra em conta nenhuma.
-- =========================================================================
