-- =========================================================================
-- O valor do orçamento vem do ORÇAMENTO, não de um campo que ninguém preenche
--
-- Correção de um erro meu de horas atrás. Em 20260807180000 pendurei o botão
-- "usar o orçamento (R$ X)" em `projects_financeiro.sold_value` — e esse
-- campo é preenchido à mão, então na prática está zerado. O botão nunca
-- apareceu pra ninguém.
--
-- O CASO REAL, hoje, no Sul Minas:
--
--   projeto 0203  → orçamento #0313, total_value = R$ 1.650
--                   sold_value da lateral = 0,00
--
-- O cliente pediu pra trazer esse job pra nota do mês. Sem o botão, ele
-- entrou pelas HORAS: 6,91h × R$ 160 = R$ 1.105,60 no lugar dos R$ 1.650
-- combinados. R$ 544,40 a menos de subtotal — R$ 853,62 na nota, com a
-- margem e o imposto por cima. É o "os valores não estão fechando".
--
-- O erro de fundo foi meu: escolhi a fonte pela conveniência do JOIN que já
-- estava ali, em vez de perguntar onde o número realmente mora. O valor
-- vendido de um job mora no orçamento dele.
--
-- Passa a valer, nesta ordem:
--   1. `projects_financeiro.sold_value`, quando alguém preencheu à mão —
--      é uma decisão explícita e vence;
--   2. `budgets.total_value` do orçamento vinculado ao projeto.
--
-- O NÚMERO do orçamento vai junto pra tela: um botão que diz "usar o
-- orçamento #0313 — R$ 1.650,00" é conferível antes do clique; um que só
-- diz "usar o orçamento" pede fé.
--
-- Não filtro por status do orçamento de propósito. O #0313 está em 'draft'
-- e o job foi feito e entregue — o vínculo com o projeto é o que diz que
-- aquele é o orçamento dele. Esconder o valor por causa do status faria a
-- tela omitir justamente o número que resolve o fechamento.
--
-- Partiu da definição vigente (20260807180000); muda só a origem do valor
-- do orçamento em `por_projeto`.
-- =========================================================================
...
-- ---------------------------------------------------------------- medição
DO $medicao$
DECLARE achou boolean; valor numeric; numero text;
BEGIN
  PERFORM public.gerar_faturamento_mensal(date '2026-07-01');

  -- O caso concreto: o botão tem que existir pro 0203, com o número do
  -- orçamento e os R$ 1.650. Sem isto a correção é teoria.
  SELECT (x->>'orcamento_valor')::numeric, x->>'orcamento_numero'
    INTO valor, numero
    FROM public.faturamento_mensal fm, LATERAL jsonb_array_elements(fm.detalhe->'por_projeto') x
   WHERE fm.ref_mes = date '2026-07-01' AND x->>'numero' = '0203';

  IF COALESCE(valor, 0) <= 0 THEN
    RAISE EXCEPTION 'o painel continua sem o valor do orçamento do 0203 — o botão não vai aparecer';
  END IF;
  IF valor <> 1650 THEN
    RAISE EXCEPTION 'valor do orçamento do 0203 saiu % (esperado 1650)', valor;
  END IF;

  -- E nenhuma linha do painel pode ter perdido o campo.
  SELECT bool_and(x ? 'orcamento_valor') INTO achou
    FROM public.faturamento_mensal fm, LATERAL jsonb_array_elements(fm.detalhe->'por_projeto') x
   WHERE fm.ref_mes = date '2026-07-01';
  IF NOT COALESCE(achou, true) THEN RAISE EXCEPTION 'linha do painel sem orcamento_valor'; END IF;

  RAISE NOTICE '0203 traz o orçamento #% com R$ % — o botão aparece', numero, valor;
END $medicao$;
