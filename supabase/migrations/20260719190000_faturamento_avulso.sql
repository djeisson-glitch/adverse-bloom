-- =====================================================================
-- Faturamento: separa o que é do fee mensal do que é AVULSO.
--
--  O Djeisson tem projetos do mesmo cliente que NÃO entram no balde do
--  contrato — são pontuais e ele fatura à parte. Até aqui o motor somava
--  tudo do cliente no mês: as horas de um projeto pontual do Sul Minas
--  entravam na fatura mensal por horas, inflando a conta.
--
--  projects.faturamento diz de que lado o projeto está:
--    'mensal' (padrão) → entra no fechamento do mês do cliente
--    'avulso'          → fatura separado, fica FORA de toda a soma
--
--  Padrão 'mensal' porque o dia a dia é a maioria e o avulso é a exceção.
--
--  Os avulsos NÃO somem: vão no detalhe do rascunho ('avulsos'), com horas
--  e entregas do mês. Avulso que some da tela é dinheiro que não é cobrado.
-- =====================================================================

ALTER TABLE public.projects
  ADD COLUMN IF NOT EXISTS faturamento text NOT NULL DEFAULT 'mensal';

DO $ck$
BEGIN
  ALTER TABLE public.projects
    ADD CONSTRAINT projects_faturamento_ck CHECK (faturamento IN ('mensal', 'avulso'));
EXCEPTION WHEN duplicate_object THEN NULL;
END $ck$;

COMMENT ON COLUMN public.projects.faturamento IS
  'mensal = entra no fechamento mensal do cliente; avulso = faturado à parte.';

CREATE INDEX IF NOT EXISTS idx_projects_faturamento ON public.projects (client_id, faturamento);
