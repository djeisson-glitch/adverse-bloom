-- =====================================================================
-- gerar_faturamento_mensal(ref, cliente?) — monta o rascunho de faturamento
-- de um mês para cada cliente com modelo de cobrança configurado.
--
-- Conta (aditiva, transparente): subtotal (conforme o modelo) → + margem%
-- (sobre o subtotal) → + imposto% (sobre subtotal+margem) = total.
--   • horas    → subtotal = (horas_edição + horas_alteração) × valor_hora
--   • tabela   → subtotal = Σ preço do tipo de cada entrega do mês
--   • contrato → subtotal = valor_mensal fixo (+ consumo de franquia no detalhe)
-- O detalhe jsonb carrega o relatório (demandas do mês e QUEM pediu,
-- alterações e quem pediu, horas por projeto) e a comparação de saúde
-- (quanto renderia por horas × nosso valor-hora de tabela).
-- =====================================================================
...
BEGIN
  ...
  LOOP
    ...
    WHERE p.client_id = c.client_id AND da.created_at >= _ini AND da.created_at < _fim;
    ...
    FROM (
      ...
      FROM public.time_entries te
      JOIN public.projects p ON p.id = te.project_id
      WHERE p.client_id = c.client_id AND te.billable AND te.start_at >= _ini AND te.start_at < _fim
      GROUP BY p.name
    ) q;
    ...
    ELSIF c.modelo = 'contrato' THEN
      ...
      WHERE p.client_id = c.client_id AND s.tipo = 'diaria' AND s.status <> 'cancelada'
      ...
      WHERE p.client_id = c.client_id AND d.data_entrega >= _ini AND d.data_entrega < _fim
        AND d.status NOT IN ('reprovado', 'cancelado');
      ...
      FROM public.deliverables d JOIN public.projects p ON p.id = d.project_id
      WHERE p.client_id = c.client_id AND d.data_entrega >= _jan_ini AND d.data_entrega < _fim
        AND d.status NOT IN ('reprovado', 'cancelado');
...
-- ---------- Cron: todo dia 01 gera o faturamento do mês anterior ---------
DO $cron$
BEGIN
  PERFORM cron.unschedule('faturamento-mensal');
EXCEPTION WHEN OTHERS THEN NULL;
END $cron$;

SELECT cron.schedule(
  'faturamento-mensal',
  '0 6 1 * *',   -- dia 1, 06:00 UTC (03:00 BRT)
  $$ SELECT public.gerar_faturamento_mensal((date_trunc('month', now()) - interval '1 month')::date, NULL, true); $$
);
