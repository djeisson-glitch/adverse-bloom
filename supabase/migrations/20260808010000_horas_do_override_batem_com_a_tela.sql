-- =========================================================================
-- As horas que saem da conta têm que ser as mesmas que a tela mostra
--
-- Bug meu, achado medindo o fechamento do Sul Minas que o Djêisson reportou.
-- Ao aplicar o valor do orçamento num job, o valor entrava certo mas as
-- horas dele saíam do cálculo por um número diferente do exibido:
--
--   a tela mostra          6,91h   (edição e alteração arredondadas a 2 casas)
--   a subtração usava      6,9167h (minutos ÷ 60, sem arredondar)
--
-- Resultado: 21,04 − 6,91 = 14,13 na calculadora de quem confere, e 14,12 na
-- conta do sistema. R$ 1,07 num fechamento. Pouco dinheiro, e exatamente o
-- tipo de diferença que faz o cliente perguntar se o resto está certo — que
-- é justamente o que estamos tentando encerrar.
--
-- Passa a arredondar do MESMO jeito que `_h_tot` é composto: edição e
-- alteração separadas, cada uma a duas casas, e só então somadas. Não é a
-- forma "mais precisa" — é a forma CONSISTENTE, que é o que vale quando o
-- número vai pra uma nota fiscal e alguém vai conferir na mão.
--
-- Partiu da definição vigente (20260807230000).
-- =========================================================================
...
DO $medicao$
DECLARE
  cli uuid; p203 uuid; s0 numeric; s1 numeric; esperado numeric;
BEGIN
  ...
  SELECT id INTO p203 FROM public.projects WHERE numero = '0203';
  IF cli IS NULL OR p203 IS NULL THEN RAISE NOTICE 'sem o caso de teste'; RETURN; END IF;

  PERFORM public.gerar_faturamento_mensal(date '2026-07-01', cli);
  SELECT subtotal INTO s0 FROM public.faturamento_mensal
   WHERE client_id = cli AND ref_mes = date '2026-07-01';

  UPDATE public.projects SET valor_fechamento = 1650, valor_fechamento_origem = 'orcamento'
   WHERE id = p203;
  PERFORM public.gerar_faturamento_mensal(date '2026-07-01', cli);
  SELECT subtotal INTO s1 FROM public.faturamento_mensal
   WHERE client_id = cli AND ref_mes = date '2026-07-01';

  -- desfaz o teste; quem aplica de verdade é a migration seguinte
  UPDATE public.projects SET valor_fechamento = NULL, valor_fechamento_origem = NULL
   WHERE id = p203;

  -- 1650 entra, 6,91h × 160 = 1105,60 sai. Delta esperado: +544,40 — o mesmo
  -- que qualquer um faz na calculadora com os números da tela.
  esperado := 1650 - 1105.60;
  IF ROUND(s1 - s0, 2) <> esperado THEN
    RAISE EXCEPTION 'delta saiu % e a calculadora dá % (sub % → %)', ROUND(s1-s0,2), esperado, s0, s1;
  END IF;

  RAISE NOTICE 'a conta bate com a tela: % → % (delta %)', s0, s1, ROUND(s1-s0,2);
END $medicao$;
