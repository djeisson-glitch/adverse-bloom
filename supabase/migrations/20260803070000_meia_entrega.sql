-- =========================================================================
-- Meia entrega: 50% quando teve trabalho, mas não o trabalho inteiro
--
-- Ontem eu tinha feito "não cobrar" pro caso do corte de podcast. O Djêisson
-- corrigiu, e com razão:
--
--   "acho que não cobrar não é o ideal. ideal seríamos cobrar 50% do valor,
--    já foi dedicado horas."
--
-- Recorte de material já editado não é brinde nem entrega cheia: alguém
-- sentou, cortou, legendou e exportou. Zero apaga esse trabalho da conta;
-- cheio cobra como se fosse peça nova.
--
-- Vira uma proporção (`cobranca_percent`) que multiplica o preço do tipo:
--   100  entrega cheia
--    50  meia — recorte, versão, adaptação de algo já editado
--     0  não cobra (segue existindo, pra brinde e cortesia)
--
-- A proporção é da PEÇA e independe do tipo: uma Pílula pela metade são
-- R$ 220, um Vídeo principal pela metade são R$ 625. Quem revisa o
-- fechamento escolhe as duas coisas — o que é, e quanto disso se cobra.
-- =========================================================================

ALTER TABLE public.deliverables
  ADD COLUMN IF NOT EXISTS cobranca_percent numeric(5,2) NOT NULL DEFAULT 100;
COMMENT ON COLUMN public.deliverables.cobranca_percent IS
  'Quanto do preço do tipo se cobra: 100 cheio, 50 meia (recorte de algo já editado), 0 cortesia.';

-- Quem tinha sido marcado com o sentinela antigo vira proporção 0.
UPDATE public.deliverables
   SET cobranca_percent = 0, tipo_cobranca = NULL
 WHERE tipo_cobranca = 'nao_cobrar';
