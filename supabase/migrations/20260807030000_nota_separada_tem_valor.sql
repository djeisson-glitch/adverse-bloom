-- =========================================================================
-- A nota separada ganha VALOR (e passa a valer por peça)
--
-- Segunda metade do pedido do Djêisson (07/08/2026). A migration anterior
-- criou `deliverables.faturamento` e a view que resolve o balde por peça;
-- esta ensina o fechamento a usar a view — e a dizer QUANTO é o que saiu.
--
-- O QUE MUDA NA CONTA DO MÊS
--
-- Em toda parte onde o fechamento perguntava "este PROJETO é mensal?", ele
-- passa a perguntar "esta PEÇA é mensal?" via `deliverables_faturamento`:
-- horas, horas por projeto, alterações, itens da tabela e as duas contagens
-- de entregas do contrato (mês e janela). São seis lugares, e é por isso que
-- a regra virou view em vez de um COALESCE copiado seis vezes.
--
-- As DIÁRIAS continuam seguindo o projeto, de propósito: uma saída de campo
-- é do job inteiro e não há a que peça atribuí-la. Está escrito no corpo,
-- nas duas consultas.
--
-- Hora SEM peça segue o projeto — não há quem decida por ela.
--
-- O BLOCO "FATURAR À PARTE"
--
-- Antes: os projetos com `faturamento = 'avulso'`, com horas e entregas.
-- Agora: agrupado por projeto, com valor, separando as duas origens —
-- projeto inteiro avulso, ou peças soltas de um projeto mensal — e com a
-- lista das peças. Preço = horas × valor-hora do cliente (decisão dele
-- hoje); sem valor-hora cadastrado, cai no rate_card de Edição e o detalhe
-- declara qual régua foi usada.
--
-- INVARIANTE que a medição no fim confere: para todo cliente, as horas do
-- mês + as horas que saíram pra nota separada = as horas faturáveis do mês
-- daquele cliente. Os dois baldes particionam; nada pode cair nos dois nem
-- sumir dos dois. Era esse o defeito que eu queria impossibilitar aqui.
--
-- Partiu da definição vigente (20260806140000) — o corpo inteiro veio de lá,
-- só os pontos acima mudaram.
-- =========================================================================
...
BEGIN
  ...
  LOOP
    ...
    WHERE p.client_id = c.client_id AND te.billable
      AND COALESCE(df.faturamento_efetivo, p.faturamento, 'mensal') = 'mensal'
    ...
    FROM (
      SELECT jsonb_build_object(
               ...
               'valor', ROUND((h.h_edic + h.h_alt) * _vh_avulso, 2),
               'entregas', e.n,
               'pecas', e.lista
             ) AS x
    ...
    ELSIF c.modelo = 'tabela' THEN
      ...
      FROM (
        ...
        WHERE p.client_id = c.client_id AND dfp.faturamento_efetivo = 'mensal'
...
-- ---------------------------------------------------------------- medição
-- Confere a PARTIÇÃO em julho/2026: o que entra no mês mais o que sai pra
-- nota separada tem que dar exatamente as horas faturáveis do cliente no
-- mês. Se um dia alguém esquecer um filtro, é aqui que a migration para —
-- e não na fatura do cliente.
DO $medicao$
DECLARE
  r record; mes date := date '2026-07-01'; fim date := date '2026-08-01';
  no_mes numeric; a_parte numeric; total numeric; n int := 0;
BEGIN
  ...
  LOOP
    SELECT
      COALESCE(SUM(te.duration_min) FILTER (
        WHERE COALESCE(df.faturamento_efetivo, p.faturamento, 'mensal') = 'mensal'), 0),
      COALESCE(SUM(te.duration_min) FILTER (
        WHERE COALESCE(df.faturamento_efetivo, p.faturamento, 'mensal') = 'avulso'), 0),
      COALESCE(SUM(te.duration_min), 0)
      INTO no_mes, a_parte, total
    FROM public.time_entries te
    JOIN public.projects p ON p.id = te.project_id
    LEFT JOIN public.deliverables_criacao dc     ON dc.id = te.deliverable_id
    LEFT JOIN public.deliverables_faturamento df ON df.id = te.deliverable_id
    ...
    IF no_mes + a_parte <> total THEN
      RAISE EXCEPTION 'particao quebrada em %: % no mes + % a parte <> % faturaveis',
        r.name, no_mes, a_parte, total;
    END IF;
    IF total > 0 THEN
      n := n + 1;
      RAISE NOTICE '% · julho/2026: %h no fechamento, %h pra nota separada',
        r.name, ROUND(no_mes/60.0, 2), ROUND(a_parte/60.0, 2);
    END IF;
  END LOOP;
  RAISE NOTICE 'particao conferida em % cliente(s) com horas em julho/2026', n;
END $medicao$;
