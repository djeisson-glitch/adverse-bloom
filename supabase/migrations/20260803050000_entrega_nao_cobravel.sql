-- =========================================================================
-- Entrega que não se cobra
--
-- "temos edições simples, como story por exemplo, não acho justo cobrar
--  R$ 446,00 do cliente" — Djêisson, 03/08/2026. Concordo, e o fechamento de
-- julho do Sicredi Região mostra o tamanho disso: 14 das 26 entregas têm
-- menos de UMA hora apontada, e seis cortes de podcast com 1–2 minutos cada
-- viravam R$ 440 por cabeça.
--
-- A causa não é o sistema classificar mal: é que ENTREGÁVEL e ITEM COBRÁVEL
-- viraram a mesma coisa. Um podcast rende um vídeo principal e seis cortes —
-- sete entregas, uma cobrança. Faltava poder dizer isso.
--
-- `tipo_cobranca = 'nao_cobrar'` é essa frase. A peça continua no relatório
-- do cliente (ele vê o que recebeu), com preço zero e marcada. Some da conta,
-- não da vista — entrega que some da tela é entrega que ninguém confere.
--
-- Ganha de nome e de horas de propósito: é decisão humana, e decisão humana
-- não se sobrepõe por heurística.
-- =========================================================================
    ...
    ELSIF c.modelo = 'tabela' THEN
      ...
      FROM (
        SELECT d.id, d.titulo, d.formato, d.duracao, d.data_entrega,
               -- "não cobrar" é decisão explícita e ganha de tudo: a peça
               -- existe, aparece no relatório do cliente, e vale zero. É o
               -- caso do corte de podcast que já está pago dentro do
               -- principal, ou do story que não se cobra avulso.
               CASE WHEN d.tipo_cobranca = 'nao_cobrar' THEN NULL
                    ELSE COALESCE(esc.tipo, nome.tipo, hr.tipo) END        AS tipo,
               CASE WHEN d.tipo_cobranca = 'nao_cobrar' THEN 0
                    ELSE COALESCE(esc.preco, nome.preco, hr.preco) END     AS preco,
               CASE WHEN d.tipo_cobranca = 'nao_cobrar' THEN NULL
                    ELSE COALESCE(esc.horas_ref, nome.horas_ref, hr.horas_ref) END AS horas_ref,
               CASE WHEN d.tipo_cobranca = 'nao_cobrar' THEN 'nao_cobrar'
                    WHEN esc.tipo IS NOT NULL THEN 'escolhido'
