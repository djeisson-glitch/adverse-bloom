-- =========================================================================
-- Diária vira valor, e saldo abate a conta
--
-- Duas coisas do mesmo pedido:
--
--  1. As diárias apareciam no detalhe do fechamento mas NÃO somavam no valor.
--     Dia de campo é serviço prestado, não anotação: o repasse (custo ×
--     margem própria × imposto quando cabe) entra no subtotal.
--
--  2. "se caso houver saldo, descontar". O saldo A USAR do cliente — o mesmo
--     extrato da ficha — abate o total do mês. Só o que couber: saldo maior
--     que a fatura não vira crédito negativo, a sobra continua no extrato pro
--     mês seguinte.
--
-- O detalhe passa a carregar `diarias_repasse` e `saldo` (disponível, usado,
-- sobra) — sem isso o cliente veria o total mudar sem saber por quê.
--
-- Partido da definição VIGENTE (20260803130000), montado por script.
-- =========================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- Diárias entram no valor: o dia de campo é serviço prestado, não
    -- anotação. O repasse já vem calculado no detalhe (margem própria +
    -- imposto quando cabe) — aqui ele soma.
    SELECT COALESCE(SUM(
