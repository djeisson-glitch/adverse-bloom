-- =========================================================================
-- O fechamento aprende o terceiro balde
--
-- `mensal_separado`: o mesmo dia a dia, com o MESMO preço do mês, em nota
-- própria. A migration anterior abriu os CHECKs; esta ensina a função.
--
-- COMO O PREÇO SAI, por modelo do cliente:
--   horas     horas do balde × valor-hora do cliente
--   tabela    preço de tabela das peças do balde — a mesma escada de
--             matching, rodada UMA vez, com FILTER decidindo a nota
--   contrato  por hora: a mensalidade paga o mês inteiro e não se divide.
--             A tela declara essa exceção; não consome franquia (as
--             contagens de diária e entrega já filtram só 'mensal').
--
-- Margem e imposto: os mesmos percentuais do cliente sobre o subtotal da
-- nota. Comissão de valor FIXO fica no fechamento principal — repeti-la em
-- cada nota cobraria duas vezes. Hoje isso não muda número nenhum (o único
-- cliente com comissão tem preços finais, onde ela sai por dentro).
--
-- O `total` da nota NÃO entra no total da fatura: é outro documento.
--
-- POR QUE UMA VARREDURA SÓ, e não uma consulta por balde: duas consultas
-- quase iguais são duas chances de o filtro de competência divergir, e aí a
-- hora cai numa nota e some da outra. As agregações separam por FILTER.
--
-- INVARIANTE que a medição confere: mensal + nota do mês + à parte = todas
-- as horas faturáveis do cliente no mês. Três baldes, partição exata.
--
-- Partiu da definição vigente (20260807030000).
-- =========================================================================
...
BEGIN
  ...
  LOOP
    ...
    IF c.modelo = 'horas' THEN
      ...
      _sep_subtotal := ROUND(_h_tot_sep * c.valor_hora, 2);
      _sep_regra    := 'horas × valor-hora do cliente';
...
-- ---------------------------------------------------------------- medição
-- A partição agora é de TRÊS baldes. Se um filtro ficar pra trás, a hora
-- some de todas as notas — e some calada, que é o pior jeito de errar numa
-- cobrança.
DO $medicao$
DECLARE
  r record; mes date := date '2026-07-01'; fim date := date '2026-08-01';
  a numeric; b numeric; d numeric; total numeric; n int := 0;
BEGIN
  ...
  LOOP
    ...
    IF total > 0 THEN
      n := n + 1;
      RAISE NOTICE '% julho/2026: mes=%h notaMes=%h aparte=%h',
        r.name, ROUND(a/60.0,2), ROUND(b/60.0,2), ROUND(d/60.0,2);
    END IF;
  END LOOP;
  RAISE NOTICE 'particao de 3 baldes conferida em % cliente(s)', n;
END $medicao$;
