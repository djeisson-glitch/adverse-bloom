-- =========================================================================
-- Um dia é um dia, e a diária tem custo
--
-- Duas correções no fechamento, do mesmo pedido:
--
--  1. A contagem de diárias era COUNT(*) das saídas. Dois projetos do mesmo
--     cliente gravados no MESMO DIA viravam duas diárias na franquia e na
--     cobrança — e a equipe tinha saído uma vez. Agora agrupa por dia e vale
--     a MAIOR fração (cheia ganha de meia).
--
--  2. Os custos do dia (logística, alimentação, hospedagem) não existiam no
--     fechamento. Entram no detalhe com o repasse já calculado: margem
--     própria de 15% — menor que a de produção, porque é repasse e não
--     trabalho — e o imposto do cliente por cima. Cliente com preço final
--     não leva imposto, como no resto da conta.
--
-- Partido da definição VIGENTE (20260803070000), montado por script.
-- =========================================================================
