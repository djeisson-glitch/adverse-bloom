-- =========================================================================
-- O fechamento passa a cortar o mês pela DATA DE CRIAÇÃO
--
-- Decisão do Djêisson (05/08/2026), depois de ver o #20263006_DIA_INTERNACIONAL
-- (criado 30/06) cobrado em julho porque as 2h17 dele foram apontadas em julho:
--
--   "o que quero é que o sistema considere apenas a data de criação,
--    independente se editamos naquele mês ou não, isso porque levamos a data
--    de criação como referência pra fazer os fechamentos."
--
-- Até aqui o fechamento por horas somava o que foi APONTADO no mês. Passa a
-- somar o que foi apontado nos JOBS DO MÊS — todas as horas deles, tenham
-- sido lançadas quando tiverem sido. O mesmo corte que a tela Entregas do mês
-- já usava: agora as duas contam a mesma coisa.
--
-- A CONSEQUÊNCIA, que é real e precisa estar escrita: um fechamento deixa de
-- ser definitivo no dia 01. Hora lançada em agosto num job de julho entra no
-- fechamento de JULHO, e o rascunho de julho cresce depois de gerado. Antes de
-- faturar, regerar o mês. (Medido hoje: o [0224]_CAPTACOES do Sicredi Região é
-- exatamente esse caso — job de julho com 4h38 lançadas em agosto.)
--
-- O que MUDA de critério:
--   • horas (modelo 'horas')          start_at   → criação
--   • entregas (modelo 'tabela')      data_entrega → criação
--   • franquia de entregas (contrato) data_entrega → criação
--   • avulsos (o bloco "faturar à parte")
--
-- O que NÃO muda, de propósito:
--   • DIÁRIAS continuam pela data da saída. Uma diária é um dia de equipe em
--     campo — tem data própria, e é por dia que a franquia do contrato é
--     medida. Jogar a diária de 30/07 no mês de um projeto de junho mudaria a
--     natureza da coisa, e não foi isso que se pediu.
--   • DEMANDAS e ALTERAÇÕES do relatório seguem pela data do pedido: elas
--     respondem "o que o cliente pediu neste mês", que é outra pergunta.
--
-- A data de criação é a mesma regra do resto do sistema: `criacao_efetiva` da
-- peça (onde a data do projeto prevalece sobre a da peça); hora lançada direto
-- no projeto, sem peça, cai na criação do projeto.
--
-- Partiu da definição vigente do 20260805210000.
-- =========================================================================
