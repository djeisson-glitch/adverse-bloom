-- =========================================================================
-- Comissão sobre o subtotal 2 no faturamento do cliente
--
-- Pedido do Djêisson (02/08/2026): o fechamento mensal precisa aceitar
-- comissão, como a planilha de orçamento já aceita. A conta passa a ser:
--
--   subtotal → + margem (= SUBTOTAL 2) → + comissão (sobre o subtotal 2)
--            → + imposto (sobre subtotal 2 + comissão)
--
-- A comissão entra ANTES do imposto de propósito: ela vai na nota, então é
-- tributada junto. Fora isso o imposto ficaria por baixo do que se paga.
--
-- O formato é o MESMO dos orçamentos ({nome, tipo: '%'|'R$', valor}) — duas
-- gramáticas de comissão no mesmo sistema seria pedir divergência.
--
-- Cliente sem comissão fica com '[]' e a conta não muda em nada.
--
-- Partido da definição VIGENTE (20260802150000), não de cópia antiga — foi
-- exatamente esse atalho que derrubou os avulsos ontem.
-- =========================================================================

ALTER TABLE public.client_faturamento
  ADD COLUMN IF NOT EXISTS comissoes jsonb NOT NULL DEFAULT '[]'::jsonb;
COMMENT ON COLUMN public.client_faturamento.comissoes IS
  'Comissões sobre o subtotal 2: [{"nome","tipo":"%"|"R$","valor"}]. Mesmo formato de budgets.comissoes.';

ALTER TABLE public.faturamento_mensal
  ADD COLUMN IF NOT EXISTS comissao_valor numeric(14,2) NOT NULL DEFAULT 0;
...
BEGIN
  ...
  LOOP
    ...
    _imposto := ROUND((_subtotal + _margem + _comissao) * COALESCE(c.imposto_percent, 0) / 100, 2);
    ...
    INSERT INTO public.faturamento_mensal AS fm (
      ...
      _subtotal, COALESCE(c.margem_percent, 0), _margem, _comissao, COALESCE(c.imposto_percent, 0), _imposto, _total,
