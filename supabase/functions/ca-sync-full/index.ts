...
serve(async (req) => {
  ...
  try {
    ...
    if (!authRow)
      return new Response(JSON.stringify({ error: "Tokens nao encontrados." }), {
        status: 200,
    ...
    if (!tokenData.access_token) {
      return new Response(JSON.stringify({ error: "Token falhou", detail: tokenData }), {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    await supabase.from("conta_azul_cache").upsert(
      {
        ...
        payload: { ...authRow.payload, ...tokenData },
